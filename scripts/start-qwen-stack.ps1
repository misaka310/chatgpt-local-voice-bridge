[CmdletBinding()]
param(
  [string]$ComfyRunBat = "D:\05_ComfyUI_ZImage\run.bat",
  [string]$ComfyBaseUrl = "http://127.0.0.1:8190",
...
$StackStatePath = Join-Path $RuntimeDir "qwen-stack.json"
...
function Save-StackState {
  ...
  $state | ConvertTo-Json -Depth 5 | Set-Content -Encoding UTF8 -Path $StackStatePath
}
...
function Stop-StartedProcesses {
  ...
  Write-Host "Stopping Qwen stack processes started by this script..."
...
function Assert-HealthEngine {
  ...
  if ($health.engine -ne "comfyui_qwen3") {
    throw ("local-api engine must be comfyui_qwen3 but was: {0}" -f $health.engine)
  }
}
...
try {
  if (-not (Test-Path -LiteralPath $ComfyRunBat)) {
    throw ("ComfyUI run.bat not found: {0}" -f $ComfyRunBat)
  }
  ...
  Write-Host ""
  Write-Host "Qwen stack is ready."
