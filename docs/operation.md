...
日常的な音声操作はLocal Voiceパネルだけで行います。パネルには`Ref`、`Volume`、`Auto`、`Next`、`Regen`、`Replay`だけを表示します。`Voice`はIrodori v3 direct固定、通常のタブ切り替えでは現在表示しているChatGPTタブを操作対象として自動判定します。Chromeの分割表示では、各ペインの定期通知だけでパネルが左右へ往復しません。デスクトップペットは`Ref`と自動連動します。
...
- `Auto`: オンにした後で新しく表示されたassistant返答だけを対象にします。オンにする前から表示されていた返答と、`思考中`・`考え中`・`Thinking`だけの途中状態は読みません。返答全文ではなく、最大2行・80文字の冒頭プレビューだけを一度再生します。
...
- Auto後の新しい返答だけを対象にし、`思考中`などの途中状態を送信せず、冒頭プレビューが最大2行・80文字であること
- `Next`、`Regen`、`Replay`の既存動作が維持されること
- 通常の複数タブでは現在アクティブなタブへ切り替わり、分割表示の定期通知だけではLocal Voiceパネルが別ペインへ移動しないこと
- 通知領域にペット用操作がないこと
